...
mongoose.connect("mongodb+srv://greendy:1089599434@db-deiby.i46ttfb.mongodb.net/");
...
db.once('open', function () {
  ...
  //Obtener a todos los usuarios que ganen más de $60000 al mes y tengan menos de 30 años.

  app.get("/api/personas/salario", async (req, res) => {
    const ejercicio3 = await personas.find({ salario: { $gte: 60000 }, edad: { $lt: 30 } });
    res.json(ejercicio3);
  });

  //Obtener a todos los usuarios que sean de España y ganen más de $70000 al mes.

  app.get("/api/personas/españa", async (req, res) => {
    const ejercicio4 = await personas.find({ ciudad: "españa", salario: { $gte: 70000 } });
    res.json(ejercicio4);
  });
  ...
  // Obtener a todos los usuarios que ganen menos de $60000 al mes o que tengan más de 40 años.

  app.get("/api/personas/salario", async (req, res) => {
    const ejercicio10 = await personas.find({ $or: [{ salario: { $lt: 60000 } }, { edad: { $gte: 40 } }] });
  ...
  app.get("/api/personas/canada", async (req, res) => {
    const ejercicio11 = await personas.find({ ciudad: "Canada", $or: [{ salario: { $gte: 4000 } }, { altura: { $gte: 180 } }] });
  ...
  //Obtener todos los usuarios que sean de Alemania y que tengan un salario menor a $80000 o una edad mayor a 35 años.

  app.get("/api/personas/alemania", async (req, res) => {
    const ejercicio18 = await personas.find({ ciudad: "Alemania", $or: [{ salario: { $lt: 80000 } }, { edad: { $gte: 35 } }] });
